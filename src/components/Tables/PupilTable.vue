  ...
  <div>
    ...
    >
      ...
      >
        ...
        <md-table-cell
          ><i class="fas fa-edit" @click="editPupil(item.id)"></i
        ></md-table-cell>
        <md-table-cell
...
export default {
  ...
  mounted() {
    this.getPupil(this.currentPage - 1, 3);
  },
  computed: {
    rows() {
  ...
  methods: {
    ...
    getPupil(page, size) {
      axios.get("/orders?page=" + page + "&size=" + size).then(res => {
        this.pupils = res.data.object
        this.searched = this.pupils;
    ...
    editPupil(id) {
      let item = this.pupils.find(item => item.id === id);
      this.param.id = id;
    ...
    searchOnTable() {
      this.searched = searchByName(this.pupils, this.search);
    },
    onPagination(pagination) {
      this.getPupil(pagination - 1, 3);
    }
  }
};
</script>
